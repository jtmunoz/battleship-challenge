...
class Gameboard
	...
	def add_hidden_spots(pos)
		if pos.to_s.length > 1
			self.hidden_board[pos.to_s[0].to_i][pos.to_s[1].to_i] = "X"
		else
			self.hidden_board[0][pos] = "X"
		end
	end
	...
	def position_ships
		...
		continue_placing_ship_horizontally(fleet.fleet_ships[:carrier])
	end
